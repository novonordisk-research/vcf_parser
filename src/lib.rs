...
mod tests {
    ...
    #[test]
    fn test_logic_expression_to_json() -> Result<(), Box<dyn Error>> {
        let tests = [
            ("canonical = YES", serde_json::json!({"name": "canonical", "op": "=", "value": "YES"})),
            ("canonical == YES and (symbol in (aaa,bbb) or cadd >20)", serde_json::json!({
                "AND": [
                    {"name": "canonical", "op": "==", "value": "YES"},
                    {"OR": [
                        {"name": "symbol", "op": "in", "value": ["aaa", "bbb"]},
                        {"name": "cadd", "op": ">", "value": 20}
                        ]}
                ]
            })),
            ("(af <= 0.01 and (impact == high or impact == moderate)) and (cadd >=20 or pangolin >= 0.5)",
            serde_json::json!({
                "AND": [
                    {
                    "name": "af",
        ...
        for (expr, expected) in tests.iter() {
            let json_obj = utils::logical_expression_to_json(expr);
